...
//         <p>No emergency services available in your area. Please try again later or call our hotline.</p>
//       </div>
//     );
//   }
...
//             </Typography>
//             <Typography variant="body2" sx={{ mt: 1, color: 'text.secondary' }}>
//               Provide your details for immediate assistance
//             </Typography>
//           </Box>
...
const Emergency = () => {
  ...
  const handleOpenModal = (service) => {
    ...
    setCurrentService(service);
    setOpenModal(true);
    // Set default to immediate service
    setIsImmediate(true);
    const now = new Date();
  ...
  const handleBooking = async () => {
    ...
    if (isImmediate) {
      ...
      maxDate.setMonth(maxDate.getMonth() + 1); // Emergency services can only be scheduled up to 1 month in advance
      ...
      >
        <Box sx={modalStyle}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            {/* Scheduling Fields - shown only when not immediate */}
            {!isImmediate && (
              <>
                <TextField
                  fullWidth
