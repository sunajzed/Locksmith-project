...
//         return (
//           serviceName.includes(query.toLowerCase()) ||
...
//                   {currentService.service.custom_price ||
//                     currentService.service.total_price}{" "}
//                   + ${additionalKeyPrice} × {additionalKeys} + 10% fee + $40
//                   platform fee)
//                 </Typography>
//               )}
//             </Box>
...
//       <h3>
//         {highlightSearchTerm(service.service.admin_service_name, searchQuery)}
//       </h3>
...
//       {highlightSearchTerm(service.service.details, searchQuery)}
//     </p>
//     <button
...
const Residential = () => {
  ...
  const validateHouseNumber = (value) => {
    return value && /^[0-9A-Za-z\s/-]+$/.test(value);
  };

  const handleHouseNumberChange = (e) => {
    const value = e.target.value;
    setHouseNumber(value);
    if (value && !validateHouseNumber(value)) {
      setHouseNumberError(
        "Please enter a valid house number (letters, numbers, spaces, hyphens)"
      );
    } else {
      setHouseNumberError("");
    }
  ...
  const handleBooking = async () => {
    ...
    if (!validateHouseNumber(houseNumber)) {
      setHouseNumberError("Please enter a valid house number");
      setBookingError("Please correct the errors above");
      return;
    }
    ...
    formData.append("locksmith", currentService.locksmith_id);
    formData.append("scheduled_date", bookingDateTime);
    formData.append("customer_address", `${houseNumber} ${address}`);
        ...
        navigate("/confirm-payment", {
          state: {
            ...
            service: {
              ...
              scheduled_date: bookingDateTime,
              isEmergency,
              customer_address: `${houseNumber} ${address}`,
  ...
  const debouncedSearch = debounce(async (query) => {
    ...
    try {
      const matchingServices = services.filter((service) => {
        const lockDetails =
          service.lock_details || service.service?.lock_details || {};
        return (
          (lockDetails.lock_type &&
            lockDetails.lock_type
              .toLowerCase()
              .includes(query.toLowerCase())) ||
          (lockDetails.manufacturer &&
            lockDetails.manufacturer
              .toLowerCase()
              .includes(query.toLowerCase())) ||
          (service.service.admin_service_name &&
            service.service.admin_service_name
              .toLowerCase()
              .includes(query.toLowerCase()))
        );
  ...
  return (
    <Box className="residential-container">
      ...
      <div className="search-container">
        <input
          type="text"
          placeholder="Search by lock type, manufacturer, or service name"
          value={searchQuery}
          onChange={handleSearchInputChange}
          className="search-input"
        />
        ...
        <Box sx={modalStyle}>
              ...
              <>
                ...
                {needMoreKeys && (
                  <Box sx={{ mt: 2 }}>
                    ...
                    />
                    <Typography variant="body2" sx={{ mt: "text.secondary" }}>
                      Additional keys: ${additionalKeyPrice} each (first key
                      included in base price)
                    </Typography>
                  </Box>
          ...
          >
            ...
            <Button
              variant="contained"
              onClick={handleBooking}
              disabled={loading || !!phoneError || !!houseNumberError}
              sx={{
                textTransform: "none",
                px: 3,
                borderRadius: "8px",
