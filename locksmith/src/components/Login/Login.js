...
export default function Login() {
  ...
  const [showPassword, setShowPassword] = useState(false);
  const [agreeToTerms, setAgreeToTerms] = useState(false);
  const [step, setStep] = useState(1); // 1 for username/password, 2 for OTP
  const navigate = useNavigate();
  const location = useLocation();
  const queryParams = new URLSearchParams(location.search);
  const selectedRole = ["customer", "locksmith"].includes(
  ...
  const handleStep1Submit = async (e) => {
    ...
    if (!agreeToTerms) {
      setError(
        "You must agree to the Terms of Use and Privacy Policy to proceed."
      );
      return;
    }
  ...
  const handleStep2Submit = async (e) => {
    ...
    try {
      ...
      if (role !== selectedRole) {
        setError(
          `Unauthorized access! You selected "${selectedRole}", but your account is "${role}".`
        );
        return;
      }
  ...
  return (
    <div className="container-fluid loginbg d-flex align-items-center justify-content-center py-5">
      <div
        className="form-wrapper bg-white shadow-lg p-5 rounded-4 w-100 mx-3"
      ...
      >
        {step === 1 ? (
          ...
          <form
            ...
            className="login-form animate-fadein"
          >
            <h2 className="text-center fw-bold mb-4">Verify OTP</h2>
            {error && (
              <div className="alert alert-danger text-center py-2">{error}</div>
            )}
            {success && (
