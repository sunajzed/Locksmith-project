...
//       // Immediately check and navigate based on the role
//       if (role === 'customer') {
//         navigate('/');
//       } else {
//         navigate('/locksmith');
...
//   // In useEffect or in the component render, check if the user is logged in:
//   useEffect(() => {
//     const isLoggedIn = localStorage.getItem('accessToken');
//     if (isLoggedIn) {
//       // Redirect to the homepage if the user is already logged in
...
export default function Login() {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (!username) {
        // If no name from Google, ask the user to enter their name
      ...
      // Send token and selected role to the backend
      const response = await api.post('/api/google-login/', { token: idToken, role: selectedRole });

      const { access, refresh, role } = response.data;

      // Save the username, tokens, and role to localStorage
      localStorage.setItem('accessToken', access);
      localStorage.setItem('refreshToken', refresh);
      localStorage.setItem('userRole', role);
