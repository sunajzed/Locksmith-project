...
import './Signup.css';
import { FaEye, FaEyeSlash, FaCopy, FaFacebook } from 'react-icons/fa';
...
export default function UserSignup() {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          ...
          setError("Unable to retrieve location. Please enable location services or try again.");
        }
      );
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.latitude || !formData.longitude) {
      setError('Location data is required. Please enable location services and try again.');
      return;
    }
    ...
    try {
      ...
      if (providerType === 'facebook' && !email) {
        ...
        if (response.data.status === 'email_required') {
          ...
          email = prompt("Please enter your email to complete registration:");
          if (!email) {
            setError("Email is required to continue with Facebook registration.");
            return;
          }
          ...
          if (otpResponse.data.status === 'otp_sent') {
            ...
            if (!otp) {
              setError("OTP is required to complete registration.");
              return;
            }
        ...
        navigate(role === 'locksmith' ? '/locksmith' : '/');
      }
    } catch (error) {
          ...
          <form onSubmit={handleSubmit} className="signup-form animate-fadein">
            ...
            <div className="social-login mt-3 text-center">
              ...
              <div className="d-flex justify-content-center">
                <div className="d-flex gap-2 w-100">
                  <button 
                    type="button" 
                    className="btn btn-google flex-grow-1" 
                  ...
                  <button
                    type="button"
                    className="btn btn-facebook flex-grow-1"
            ...
            <div className="text-center mt-4">
              <small className="text-muted">
                Already have an account? <Link to="/login?role=customer">Login</Link>
              </small>
            </div>
          </form>
        ) : (
