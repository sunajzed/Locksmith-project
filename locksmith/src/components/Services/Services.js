...
const OurServices = () => {
  ...
  const iconMap = {
    GiHouseKeys: <GiHouseKeys size={60} color="#2c3e50" />,
    GiCarKey: <GiCarKey size={60} color="#2c3e50" />,
    FaBuildingLock: <FaBuildingLock size={60} color="#2c3e50" />,
    GiSiren: <GiSiren size={60} color="#2c3e50" />,
    FaFingerprint: <FaFingerprint size={60} color="#2c3e50" />,
  };
  ...
  return (
    <Container className="our-services-section text-center">
      <div className="mb-5">
        ...
        <p className="text-black" dangerouslySetInnerHTML={{ __html: content.description }} />
      </div>
              ...
              <Card className="service-card service-demo shadow-lg" data-aos="fade-up" data-aos-delay={index * 200}>
                <div className="icon-container">
                  <motion.div whileHover={{ scale: 1.2, rotate: 10 }} whileTap={{ scale: 0.8, rotate: -10 }}>
                    {iconMap[service.icon] || <GiKeyLock size={60} color="#2c3e50" />}
                  </motion.div>
                </div>
