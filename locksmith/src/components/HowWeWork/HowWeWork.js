...
//   const icons = [faCalendarCheck, faDollarSign, faUserLock, faToolbox, faCommentDots];
//   const carouselRef = useRef(null);
//   const [currentIndex, setCurrentIndex] = useState(0);
//   const [isMobile, setIsMobile] = useState(window.innerWidth <= 768);
...
//                 <div className="card-body">
//                   <FontAwesomeIcon icon={icons[index] || faToolbox} size="3x" className="step-icon" />
//                   <h5 className="fw-bold mt-3">{step.heading}</h5>
//                   <p className="card-text mt-2 text-black">{step.text}</p>
//                 </div>
//               </div>
    ...
    >
      <div className="container text-center py-5">
        ...
        <div className="position-relative mt-4">
          ...
          <div
            className="d-flex overflow-hidden carousel-container"
            ref={carouselRef}
          >
            {content.steps.map((step, index) => (
              <div
                key={index}
                className={`card step-card mx-2 ${
                  isMobile ? "full-width-card" : ""
                }`}
              >
                <div className="card-body">
                  <FontAwesomeIcon
                    icon={icons[index] || faToolbox}
                    size="3x"
                    className="step-icon"
                  />
                  <h5 className="fw-bold mt-3">{step.heading}</h5>
